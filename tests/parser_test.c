...
static void run_case(const char* raw_line,
    const char* exp_sym,
    const char* exp_dest,
    const char* exp_comp,
    const char* exp_jump) {
    ...
    int pass = 1;
    if (!str_eq(sym, exp_sym))
        pass = 0;
    if (!str_eq(dest, exp_dest))
        pass = 0;
    if (!str_eq(comp, exp_comp))
        pass = 0;
    if (!str_eq(jump, exp_jump))
        pass = 0;
